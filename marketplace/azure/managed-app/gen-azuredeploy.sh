#!/usr/bin/env bash
# Generate (or verify) the committed azuredeploy.json that the README's
# "Deploy to Azure" button serves via raw.githubusercontent.com.
#
...
if [ "$CHECK" = 1 ]; then
  ...
  echo "azuredeploy.json is current"
else
