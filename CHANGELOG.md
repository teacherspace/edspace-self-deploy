...
### Changed
- Azure: the Deploy-to-Azure image pin now uses the registry's bare-version tag (`1.0.2`). The `v1.0.1` pin referenced a tag that never existed in `edspace.azurecr.io` (the release pipeline strips the `v`), so every install from that button deployed "successfully" and then sat in `ImagePullBackOff`; `scripts/release-guard.sh` now rejects a `v` pin.
