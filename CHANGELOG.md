...
### Added
- First-run setup page: every packaging now provides `EDSPACE_SETUP_TOKEN` (Azure: generated by the template and printed as the `setupUrl` output; Helm: generated into the `<release>-generated` Secret and printed in the install notes; Compose: written by `scripts/generate-secrets.sh`). Opening `/setup?token=…` on a fresh database creates the first platform admin from the browser — no `az containerapp exec` / `bin/edspace rpc` step. Requires an app version with `/setup` support; `EDSPACE_PLATFORM_ADMINS` remains as the CLI fallback.
