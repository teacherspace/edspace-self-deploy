...
### Added
- Azure: an "Update EdSpace" button (`marketplace/azure/managed-app/update.json`) for self-managed instances — a deployment script that moves the Container App to the released image and waits for the new revision to become healthy. `make image-check` verifies the pinned image exists in the registry before a release.
...
### Changed
- Azure: the Deploy-to-Azure image pin now uses the registry's bare-version tag (`1.0.3`). The `v1.0.1` pin referenced a tag that never existed in `edspace.azurecr.io` (the release pipeline strips the `v`), so every install from that button deployed "successfully" and then sat in `ImagePullBackOff`; `scripts/release-guard.sh` now rejects a `v` pin.
...
### Upgrade notes
- Azure template and Helm chart require app **1.0.2 or later**: `mailerAdapter` / `MAILER_ADAPTER` (`smtp`, `none`) is read by the app from 1.0.2; 1.0.1 requires `MAILPACE_API_KEY` unconditionally and crash-loops at boot (`environment variable MAILPACE_API_KEY is missing`) when deployed with any other mailer.
