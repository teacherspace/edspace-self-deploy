...
**First admin.** A fresh install has no account that can reach the backoffice. The chart generates a first-run setup token with the other secrets (`secrets.setupToken` to set one explicitly) and the install notes print the `/setup?token=…` link: open it, enter the admin's email and password, and you are signed in as platform admin. The link is valid for seven days from the token's issue time and disappears once an admin exists. CLI fallback: set `env.EDSPACE_PLATFORM_ADMINS` to a comma-separated list of admin emails, then run `bin/edspace rpc 'Edspace.Accounts.AdminReconciler.bootstrap() |> IO.inspect(pretty: true)'` in the app container. It is additive — it never demotes an existing admin. With email enabled, the admin requests a magic link from `/sign-in`; with `mailer.adapter: none`, the command returns a seven-day `onboarding_links` URL to hand over securely so the admin can create their password.
