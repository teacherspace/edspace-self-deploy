...
| `MAILER_SMTP_USERNAME` | conditional |  |  | SMTP username. Setting it flips MAILER_SMTP_AUTH to "always" — a relay given credentials is expected to use them. Authentication keys off this variable alone, so a password without it is never used: supply both, or neither for a relay that accepts unauthenticated mail. **Required when** MAILER_ADAPTER=smtp and MAILER_SMTP_PASSWORD is set (blank counts as missing). |
...
| `EDSPACE_SETUP_TOKEN` | no |  | yes | First-run setup token. While it is set and no platform admin exists yet, the app serves `/setup?token=<value>`: a page that creates the first platform admin (email + password) and signs them in. The token is `<yyyyMMddHHmmss>.<random>` — the timestamp is its issue time, after which it is honoured for 7 days — and is ignored once any platform admin exists, so it can stay in the environment. The Azure template generates it and prints the link as the `setupUrl` deployment output; Helm and Compose installs set it themselves (`date -u +%Y%m%d%H%M%S`.`openssl rand -hex 16`). The CLI alternative is `EDSPACE_PLATFORM_ADMINS`. |
| `EDSPACE_PLATFORM_ADMINS` | conditional |  |  | Comma-separated emails granted platform-admin on reconciliation. CLI alternative to the `/setup` page (`EDSPACE_SETUP_TOKEN`) for seeding the first admin: set this and then run `bin/edspace rpc 'Edspace.Accounts.AdminReconciler.bootstrap() \|> IO.inspect(pretty: true)'` inside the app container. With email disabled, the result contains onboarding links to hand over securely. Additive only — it never demotes an existing admin. Read on the serving node. **Required when** bootstrapping the first platform admin on a fresh install without the /setup page. |
